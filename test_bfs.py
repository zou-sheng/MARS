...
def compute_last_row(data):
    ...
    # 计算前7行的乘积
    product = np.prod(data[:7], axis=0)
    
    # 获取对应的target值
    targets = np.array([target_r, target_s, target_p, target_q, target_c, target_k, target_h, target_n])
...
def maximize_row_with_constraint(data, row, constraint_func, target, objective_func, max_iterations=10000):
    ...
    while queue and iterations < max_iterations:
        ...
        for j in range(len(current_data[row])):
            ...
            for value in range(data[row][j], 0, -1):
    ...
    print(f"第{row}行最大化完成，共执行 {iterations} 次迭代")
    return best_data
...
def integerize_with_staged_optimization(data):
    ...
    def objective_row1(data):
        return np.prod(data[1])
    data = maximize_row_with_constraint(data, 1, constraint_row1, 32, objective_row1)
    print(data)
    # 第二阶段：修改第五行
    def constraint_row5(data):
        return data[5][0] * data[5][1] * data[5][2] * data[5][3] * data[5][4] * data[5][5] * data[5][6] * data[5][7]
    def objective_row5(data):
        return np.prod(data[5])
    data = maximize_row_with_constraint(data, 5, constraint_row5, 32, objective_row5)
    print(data)
    # 第三阶段：修改第0行
    def objective_row0(data):
        return data[0][0] * data[0][1] * data[0][4] * data[0][5] * data[0][6]
    data = maximize_row_with_constraint(data, 0, lambda x: compute_constraints(x)['reg_cap'], target_reg_cap, objective_row0)
    print(data)
    # 第四阶段：修改第2行
    def objective_row2(data):
        return data[0][2] * data[0][3] * data[0][5] * data[0][6] * data[0][7] * \
               data[1][2] * data[1][3] * data[1][5] * data[1][6] * data[1][7] * \
               data[2][2] * data[2][3] * data[2][5] * data[2][6] * data[2][7]
    data = maximize_row_with_constraint(data, 2, lambda x: compute_constraints(x)['acc_cap'], target_acc_cap, objective_row2)
    print(data)
    # 第五阶段：修改第3行
    def objective_row3(data):
        return data[0][0] * data[0][1] * data[0][4] * data[0][5] * data[0][6] * \
               data[1][0] * data[1][1] * data[1][4] * data[1][5] * data[1][6] * \
               data[2][0] * data[2][1] * data[2][4] * data[2][5] * data[2][6] * \
               data[3][0] * data[3][1] * data[3][4] * data[3][5] * data[3][6]
    data = maximize_row_with_constraint(data, 3, lambda x: compute_constraints(x)['wei_cap'], target_wei_cap, objective_row3)
    print(data)
    # 第六阶段：修改第4行
    def objective_row4(data):
        return data[0][0] * data[0][1] * data[0][2] * data[0][3] * data[0][4] * data[0][6] * data[0][7] * \
               data[1][0] * data[1][1] * data[1][2] * data[1][3] * data[1][4] * data[1][6] * data[1][7] * \
               data[2][0] * data[2][1] * data[2][2] * data[2][3] * data[2][4] * data[2][6] * data[2][7] * \
               data[3][0] * data[3][1] * data[3][2] * data[3][3] * data[3][4] * data[3][6] * data[3][7] * \
               data[4][0] * data[4][1] * data[4][2] * data[4][3] * data[4][4] * data[4][6] * data[4][7]
    data = maximize_row_with_constraint(data, 4, lambda x: compute_constraints(x)['inp_cap'], target_inp_cap, objective_row4)
    print(data)
    # 第七阶段：修改第6行
    def objective_row6(data):
        return data[0][2] * data[0][3] * data[0][5] * data[0][6] * data[0][7] * \
               data[1][2] * data[1][3] * data[1][5] * data[1][6] * data[1][7] * \
               data[2][2] * data[2][3] * data[2][5] * data[2][6] * data[2][7] * \
               data[3][2] * data[3][3] * data[3][5] * data[3][6] * data[3][7] * \
               ...
               data[5][0] * data[5][1] * data[5][2] * data[5][3] * data[5][4] * data[5][6] * data[5][7] * \
               data[6][0] * data[6][1] * data[6][2] * data[6][3] * data[6][4] * data[6][6] * data[6][7]
    data = maximize_row_with_constraint(data, 6, lambda x: compute_constraints(x)['gb_cap'], target_gb_cap, objective_row6)
    print(data)
    # 第八阶段：计算第7行，根据前7行乘积和目标值
